...
class HtmlResponseProcessorTest extends WordSpec {
  ...
  private var responseProcessor: HtmlResponseProcessor = _

  "An HtmlResponseProcessor" when {
    "created" should {
      "initialize" in {
        responseProcessor = new HtmlResponseProcessor
      }
    }
    "asked to process data" should {
      "return dummy" when {
        "data is emtpy" in {
          assertEquals(dummyGallery,
                       responseProcessor.processHtmlToGallery(""))
        }
        "html doesn't contain needed data" in {
          ...
          assertEquals(dummyGallery,
                       responseProcessor.processHtmlToGallery(brokenHTML))
        }
        "nhentai 404 is found" in {
          ...
          assertEquals(dummyGallery,
                       responseProcessor.processHtmlToGallery(nh404))
        }
      }
      "return data in Gallery" when {
        "full page is returned" in {
          val nhentaiHtmlFile =
          ...
          assertEquals(testGallery,
                       responseProcessor.processHtmlToGallery(nhPage))
        }
      }
    }
  }
